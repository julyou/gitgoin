...
import { Flex, Button, Spinner, Text, Box, Image, Progress } from '@chakra-ui/react'
    ...
    const handleClick = async () => {
        ...
        for (let i = 0; i < words.length && i < 5; i++) {
            ...
            for (let i = 0; i < gfiSearch2.data.items.length && i < 6; i++) {
                ...
                const langs = await axios.get(repo.data.languages_url, requestAuthConfig);
                const mainLang = repo.data.language
                console.log("REPO MAIN LANG: ", repo.data.language);
                console.log("REPO LANGS:",langs); 
                const mainLangValue = langs.data[mainLang]; 
                let sum = 0;
                for (let property in langs.data) {
                    sum += langs.data[property];
                }
                const proportion = mainLangValue/sum;
                // 
                console.log(sum);
                    ...
                    <>
                        ...
                        < Flex className={style ? "" : "grid"} direction="column" p="0px" gap="10px" >
                            {goodIssues.issues.map((item, i) => {
                                ...
                                return (<Poster key={item.url} animation={animStr(i)} western={style} repoName={item.name} topics={style ? item.topics : item.topics.slice(0, 7)} repoUrl={item.url} title={item.title} score={item.score} proportion={item.proportion} lang={item.lang} />);
